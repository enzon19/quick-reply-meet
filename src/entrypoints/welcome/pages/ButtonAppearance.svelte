	...
	<ConnectedButtons>
		<Button square label
			><input
				...
				type="radio"
				value="top"
				bind:group={buttonsPosition} />{browser.i18n.getMessage('topButtonsPositionShort')}</Button>
		<Button square label
			><input
				name="position"
				type="radio"
				value="bottom"
				bind:group={buttonsPosition} />{browser.i18n.getMessage(
				'bottomButtonsPositionShort'
			)}</Button>
	</ConnectedButtons>
	<label for="roundness">
		{browser.i18n.getMessage('settingsButtonsRoundness')}
		<Slider
